...
4. **Firebase Configuration (Required for Google Auth):**
   - Create a Firebase project at [Firebase Console](https://console.firebase.google.com/).
   - Register a web application within your project.
   - Open Firebase Authentication and enable the **Google provider**.
   - Add your development domain (`localhost`) to the Authorized Domains list.
   - Add your production domain when deployed.
   - Copy the Firebase web configuration into your `.env` file.
...
5. **Environment Variables:**
   ...
   ```
   *Note: If Firebase is not configured, the login flow will throw a clean warning instead of crashing.*

6. **Start Dev Server:**
   ```bash
   npm run dev
   ```
   Open `http://localhost:5173` (or the port Vite provides) to view it in the browser.

---

## 🌍 Deployment

This project uses Vite, so it is optimized and ready to be deployed to static hosting platforms like Vercel, Netlify, or GitHub pages.
...
3. In the Environment Variables section, add:
   ...
   - **Value:** `<your-api-key>`
4. Click **Deploy**. Vercel will automatically detect the Vite build settings (`npm run build`, `dist` output).
